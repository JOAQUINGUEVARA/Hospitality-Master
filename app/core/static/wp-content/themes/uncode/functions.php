...
add_filter( 'gettext', 'uncde_new_read_more_text', 20, 3 );
function uncde_new_read_more_text( $translated_text, $text, $domain ) {

  if ( !is_admin() ) {
      switch ( $translated_text ) {
          case 'Read More' :
              $translated_text = __( 'Más información', 'uncode' );
              break;
      }
  }
